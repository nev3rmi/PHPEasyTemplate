FR.utils = {
	...
	showPreview: function (item) {
		...
		var noImage = ['pdf', 'mp3', 'm4a'];
		if (!item.data.isFolder && item.data.thumb && noImage.indexOf(ext) == -1 && item.data.filetype != 'wvideo') {
		...
		if (ext != 'pdf') {opts.loadingMsg = FR.T('Loading...');}
		if (item.data.filetype == 'arch') {
			opts.width = 500;
			opts.height = 400;
		}
		if (ext.length > 0 && item.data.filetype == 'mp3') {
			opts.modal = false;
			opts.width = 400;
			if (FR.isMobile) {
				opts.width = 350;
			...
			opts.tools = [{
				...
				handler: function (event, toolEl, win) {
					...
					win.setSize(250, 5).collapse(false).anchorTo(FR.UI.window.el, 'bl-bl', [10, -40]);
				}
			},
			{
				...
				handler: function (event, toolEl, win) {
					...
					win.setSize(opts.width, opts.height).clearAnchor().expand(false).center();
				}
			}];
		}
		FR.UI.popup(opts);
		...
		if (treeNode) {
			...
			if (updates == 'remove') {
				...
				Ext.iterate(updates, function (k, v) {
					if (['weblink', 'star', 'share', 'sync', 'notInfo'].indexOf(k) != -1) {
...
FR.contextMenuActions = {
	...
	newFolder: function(cm) {
		...
		if (cm.location == 'tree') {
			...
			path = (cm.target[0].path || FR.currentPath+'/'+cm.target[0].filename);
		}
		new Ext.ux.prompt({
	...
	download: function(cm) {
		if (cm.location == 'tree') {
			...
			if (cm.target.length == 1) {
				var s = cm.target[0];
				FR.actions.download(s.path || FR.currentPath+'/'+s.filename, s.filename);
			} else {
				FR.UI.infoPanel.expand();
				FR.UI.cartPanel.show();
				Ext.each(cm.target, function(d) {FR.UI.cartPanel.addItem(d);}, this);
			}
		}
	},
	...
	weblink: function(cm) {
		var path, title;
		if (cm.location == 'tree') {
			path = FR.UI.tree.panel.getNodeById(cm.target.id).getPath('pathname');
			title = cm.target.text;
		} else {
			path = (cm.target[0].path || FR.currentPath+'/'+cm.target[0].filename);
			title = cm.target[0].filename;
		}
		FR.actions.WebLink(path, title);
	},
	...
	shareWithUsers: function(cm) {
		...
		if (cm.location == 'tree') {
			var n = FR.UI.tree.panel.getNodeById(cm.target.id);
			FR.sharing.path = n.getPath('pathname');
			FR.sharing.folderName = cm.target.text;
		} else {
			FR.sharing.path = (cm.target[0].path || FR.currentPath+'/'+cm.target[0].filename);
			FR.sharing.folderName = cm.target[0].filename;
		}
		FR.UI.persistentWindow({
			id: 'folderShare',
			src:FR.baseURL+'/?module=fileman_myfiles_share',
			post: [{name:'path', value: FR.sharing.path}], modal: true,
			width:500, height:380, title: FR.sharing.folderName,
	...
	props: function(cm) {
		...
		params.push({name:'path', value: path});
		FR.UI.popup({
			src:FR.baseURL+'/?module=folder_options',
			post: params,
			width:380, height:350, title: title,
			initMsg: FR.T('Loading...')
		});
	},
	comment: function() {return FR.actions.comments();},
	metadata: function(cm) {
		var s = cm.target[0];
		FR.actions.openMetadata({title: s.filename, path: (s.path || FR.currentPath+'/'+s.filename)});
	},
	...
	copy: function(cm) {
		...
		if (cm.location == 'tree') {
			...
			Ext.each(cm.target, function (s) {
				FR.copyingPaths.push(s.path || FR.currentPath+'/'+s.filename);
			});
		}
	...
	pasteCopied: function(cm) {
		...
		if (cm.location == 'tree') {
			...
			if (cm.target[0]) {
				pars.copyTo = (cm.target[0].path || FR.currentPath + '/' + cm.target[0].filename);
			}
		}
	...
	rename: function(cm) {
		...
		if (cm.location == 'tree') {
			var n = FR.UI.tree.panel.getNodeById(cm.target.id);
			pars.path = n.getPath('pathname');
			oldName = cm.target.text;
		} else {
			pars.path = (cm.target[0].path || FR.currentPath+'/'+cm.target[0].filename);
			oldName = cm.target[0].filename;
		}
		new Ext.ux.prompt({
			title: FR.T('Rename'), defaultValue: oldName,
		...
		if (cm.location == 'tree') {
			...
			if (FR.currentSection == 'trash') {
				...
				Ext.each(cm.target, function (s) {
					...
					paths.push((s.path || FR.currentPath+'/'+s.filename));
				});
				FR.removeParams['paths[]'] = paths;
			}
		}
...
FR.actions.openMetadata = function(opts) {
	FR.UI.popup({
		title: opts.title,
		width: 500, height: 400, autoDestroy: true,
...
FR.actions.comments = function(path) {
	if (!path) {
		...
		path = item.data.path ? item.data.path : FR.currentPath+'/'+item.data.filename;
	}
...
FR.actions.move = function(drop, target) {
	...
	if (drop.data.grid) {
		Ext.each(drop.data.selections, function(s) {
			...
			FR.moveParams['paths[]'].push((s.data.path || FR.currentPath+'/'+s.data.filename));
		});
	} else if (drop.tree) {
		hasFolders = true;
...
FR.actions.changeLocking = function(lock) {
	...
	var path = (FR.currentlyLockingItem.data.path || FR.currentPath+'/'+FR.currentlyLockingItem.data.filename);
...
FR.actions.UnWebLink = function() {
	...
	Ext.each(FR.UI.gridPanel.getSelectedFiles(), function(item) {
		params['paths[]'].push((item.path || FR.currentPath+'/'+item.filename));
	});
...
FR.actions.star = function(cm, action) {
	...
	if (cm.location == 'tree') {
		...
		Ext.each(cm.target, function (s) {
			params['paths[]'].push((s.path || FR.currentPath+'/'+s.filename));
		});
...
FR.actions.customAction = function(opt, path, filename) {
	...
	if (opt.popup) {
		if (path) {
			...
			sel = FR.UI.gridPanel.getSelectionModel().getSelections();
			postData = [{name: 'path', value: FR.currentPath}];
			if (sel.length == 1) {
				filename = sel[0].data.filename;
				postData.push({name:'path', value: sel[0].data.path ? sel[0].data.path : FR.currentPath+'/'+sel[0].data.filename});
			} else {
				filename = FR.T('[multiple files]');
		...
		if (opt.width) {popOpts.width = opt.width;}
		if (opt.height) {popOpts.height = opt.height;}
		if (opt.external) {
		...
		if (path) {
			...
			postData = 'path=' + encodeURIComponent(FR.currentPath);
			sel = FR.UI.gridPanel.getSelectionModel().getSelections();
			if (sel.length == 1) {
				postData += '&paths[]=' + encodeURIComponent(sel[0].data.path ? sel[0].data.path : FR.currentPath+'/'+sel[0].data.filename);
			} else {
				Ext.each(sel, function (item) {
					postData += '&paths[]=' + encodeURIComponent(item.data.path ? item.data.path : FR.currentPath + '/' + item.data.filename);
				});
			}
		}
