		...
		this.formPanel = new Ext.form.FormPanel({
			...
			items: [
				...
				{xtype: 'hidden', name: 'language', ref: 'language', value: ''},
				{
					...
					items: [
						{
							xtype: 'checkbox', checked: Settings.allowPersistentLogins, flex:1,
				...
				{xtype: 'displayfield', value: '<a href="'+signUpURL+'">'+FR.T('Register')+'</a>', style:'padding-top:5px;text-align:right', hidden: !Settings.signUpEnabled}
			]
		});
			...
			listeners: {'show': function(){
				...
				if (Settings.ui_login_logo.length) {
					...
					if (Settings.ui_login_text.length) {
						new Ext.Window({
							html: '<div class="loginTextFooter">'+Settings.ui_login_text+'</div>', autoHeight: true, unstyled: true,
...
Ext.onReady(function() {
	...
	if (Ext.isAndroid) {
		new Ext.ux.prompt({text: FR.T('There is an Android app that you can use to access your files more comfortably. Would you like to try that instead?'), confirmHandler: function() {
			document.location.href = Settings.androidAppURL;
			FR.initForm();
		}, cancelHandler: function() {FR.initForm();}});
	}
});