		...
		init: function() {
			this.actions = {
				...
				deActivate: new Ext.Action({
					text: FR.T('Deactivate'), style: 'margin-right:5px',
					iconCls: 'fa fa-fw fa-toggle-off', hidden: true,
					handler: function() {
			...
			this.gridQuickSearchField = new Ext.form.TriggerField({
				width: 100, emptyText: FR.T('Quick search'), enableKeyEvents: true,
			...
			this.panel = new FR.components.Grid({
				tbar: {
					items: [this.actions.add, this.actions.edit, this.actions.view, this.actions.openWebLink, this.actions.search, this.actions.loginAs, this.actions.viewLog, '->', {text:'&nbsp;', disabled: true}, this.actions.deActivate, this.actions.del, this.actions.archiveLogs, this.gridQuickSearchField],
					enableOverflow: true
				}
			});
		},
